...
async def test_rag_mode_one_time_reminder_preserves_daily(monkeypatch):
    ...
    try:
        ...
        # Enable persistent RAG mode for this user
        mm = MemoryManager(db)
        mm.store_memory(user_id, "rag_mode_enabled", "true", category="conversation", source="test")

        dialogue = DialogueEngine(db)

        # Verify listing reminders shows the daily 09:00
        ...
        # Monkeypatch trigger handler to create a one-time schedule when triggered
        ...
        # Send the one-time reminder while RAG is enabled
        ...
        # Verify both schedules exist and daily has not been modified
