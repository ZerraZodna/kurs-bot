    ...
    async def mock_call_ollama(prompt: str, model: str | None = None, language: str | None = None) -> str:
        ...
        if name_match:
            ...
            return json.dumps({
                ...
                "functions": [{"name": "extract_memory", "parameters": {"key": MemoryKey.FIRST_NAME, "value": name}}],
            })
        ...
        if user_msg_lower in ["yes", "ja"]:
            ...
            if current_onboarding_step == "name" or "name" in prompt_lower:
                ...
                functions = [{"name": "confirm_yes", "parameters": {"context": "use_telegram_name"}}]
                if name_from_profile:
                    functions.append({
                        "name": "extract_memory",
                        "parameters": {"key": MemoryKey.FIRST_NAME, "value": name_from_profile},
                    })
                return json.dumps({"response": response_text, "functions": functions})
            if current_onboarding_step == "timezone" or "timezone" in prompt_lower:
                ...
                return json.dumps({
                    ...
                    "functions": [{"name": "confirm_yes", "parameters": {"context": "timezone_inferred"}}],
                })
            if current_onboarding_step == "consent" or "consent" in prompt_lower:
                ...
                return json.dumps({
                    "response": response_text,
                    "functions": [{"name": "confirm_yes", "parameters": {"context": "data_consent"}}],
                })
            # Generic yes
            return json.dumps({
        ...
        if user_msg_lower in ["no", "nei"]:
            return json.dumps({
                ...
                "functions": [{"name": "confirm_no", "parameters": {}}],
            })
        ...
        if tz_match:
            ...
            return json.dumps({
                "response": response_text,
                "functions": [{"name": "set_timezone", "parameters": {"timezone": tz}}],
            })
