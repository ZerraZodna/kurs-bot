"""Unified LLM client mocking utilities for both Ollama and OpenAI providers."""
...
class LLMClientMock:
    ...
    Usage:
        mock = LLMClientMock()
        mock.set_response("lesson", "Lesson 26 content here...")
        mock.set_stream_calls("prompt", [{"name": "extract_memory", "arguments": '{"key":"current_lesson","value":"26"}'}])

        mock.patch_ollama(monkeypatch)   # for Ollama provider
        mock.patch_openai(monkeypatch)   # for OpenAI provider
    """
    ...
    def __init__(self):
        ...
        self._default_response = "[MOCK_LLM_REPLY] Default response"

    def set_response(self, prompt_contains: str, response: str) -> "LLMClientMock":
        """Set a response for prompts containing specific text."""
        self._responses[prompt_contains.lower()] = response
        return self

    def set_stream_calls(self, prompt_contains: str, tool_calls: list) -> "LLMClientMock":
        """Set tool_calls for stream_llm prompts containing specific text."""
        self._stream_calls[prompt_contains.lower()] = tool_calls
        return self

    def set_stream_lesson_response(self, prompt_contains: str, lesson_text: str) -> "LLMClientMock":
        """Set direct lesson text stream for prompts containing specific text."""
        self._stream_lessons[prompt_contains.lower()] = lesson_text
        return self

    def set_default_response(self, response: str) -> "LLMClientMock":
    ...
    def patch_openai(self, monkeypatch):
        """Patch the OpenAI client module."""
        import src.services.dialogue.openai_client as openai_module

        monkeypatch.setattr(openai_module, "call_llm", self._mock_call)
        monkeypatch.setattr(openai_module, "stream_llm", self._mock_stream)
        return self


# Keep the old OllamaMock class for backward compatibility with existing tests
class OllamaMock(LLMClientMock):
    """Backward-compatible alias for LLMClientMock (old name)."""

    def patch(self, monkeypatch):
        """Patch the Ollama client module (old method name)."""
        return self.patch_ollama(monkeypatch)
