...
def test_full_pdf_lessons_start_with_capital_lesson_header():
    pdf = Path("src/data/Sparkly ACIM lessons-extracted.pdf")
    if not pdf.exists():
...
def test_full_pdf_lesson_1_title_not_intro_bleed():
    pdf = Path("src/data/Sparkly ACIM lessons-extracted.pdf")
    if not pdf.exists():
        pytest.skip("ACIM source PDF not available in test environment")
