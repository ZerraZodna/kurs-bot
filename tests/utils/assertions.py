...
def assert_memory_stored(
    ...
    """
    query = db.query(Memory).filter_by(
        user_id=user_id,
        key=key,
        is_active=is_active
    )
    ...
    assert memory is not None, (
        f"Memory with key '{key}' not found for user {user_id} "
        f"(is_active={is_active})"
    )
    
    if expected_value is not None:
        assert memory.value == expected_value, (
            f"Memory value mismatch for key '{key}': "
            f"expected '{expected_value}', got '{memory.value}'"
        )
    
    if category is not None:
        assert memory.category == category, (
            f"Memory category mismatch for key '{key}': "
            f"expected '{category}', got '{memory.category}'"
        )
...
def assert_schedule_created(
    ...
    assert schedule is not None, (
        f"Schedule of type '{schedule_type}' not found for user {user_id} "
        f"(is_active={is_active})"
    )
...
def assert_message_logged(
    ...
    assert log is not None, (
        f"No {direction} message found for user {user_id}"
    )
    
    if content_contains is not None:
        assert content_contains in log.content, (
            f"Message content doesn't contain '{content_contains}': {log.content}"
        )
...
def assert_onboarding_step(
    ...
    assert actual_step == expected_step, (
        f"Expected onboarding step '{expected_step}', got '{actual_step}'"
    )
...
def assert_onboarding_complete(db: Session, user_id: int) -> None:
    ...
    assert is_complete, (
        f"Expected onboarding to be complete for user {user_id}, "
        f"but current_step is '{status.get('current_step')}'"
    )
...
def assert_response_contains(response: str, *expected_substrings: str) -> None:
    ...
    for substring in expected_substrings:
        assert substring in response, (
            f"Response doesn't contain '{substring}': {response[:200]}..."
        )
