...
class TestMemoryBuilder:
    ...
    def test_build_memory_with_builder(self, db_session: Session, test_user: User):
        ...
        memory = MemoryBuilder(db_session, test_user.user_id) \
            .with_key("preference") \
            .with_value("morning") \
    ...
    def test_build_many_memories(self, db_session: Session, test_user: User):
        ...
        memories = MemoryBuilder(db_session, test_user.user_id) \
            .with_category("test") \
            .build_many(5)
...
class TestScheduleBuilder:
    ...
    def test_build_daily_schedule(self, db_session: Session, test_user: User):
        ...
        schedule = ScheduleBuilder(db_session, test_user.user_id) \
            .daily() \
            .at_time(9, 0) \
            .active() \
            .build()
...
class TestConversationBuilder:
    ...
    def test_build_conversation(self, db_session: Session, test_user: User):
        ...
        messages = ConversationBuilder(db_session, test_user.user_id) \
            .user_says("Hello") \
...
class TestAssertionHelpers:
    ...
    def test_assert_memory_stored_fails_when_missing(
        self,
        db_session: Session,
        test_user: User
    ):
...
class TestParametrized:
    ...
    @pytest.mark.parametrize("time_string,expected_hour,expected_minute", [
        ("9:00 AM", 9, 0),
        ("2:30 PM", 14, 30),
        ("21:00", 21, 0),
    ])
...
class TestWithUserFixture:
    ...
    def test_user_with_memories_has_onboarding_data(
        self,
        db_session: Session,
        test_user_with_memories: User
    ):
