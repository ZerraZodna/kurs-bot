...
async def test_birthdate_memory_stored_with_real_ollama(db_session):
    """Integration test using a real Ollama instance (skips if Ollama unavailable).

    Sends the message through the real production path (`DialogueEngine.process_message`) and
    asserts that a memory containing the birthdate text was stored in the `memories` table.
    """
    ...
    # Query Memories table for any value containing a 4-digit year (e.g., 1966)
    import re
