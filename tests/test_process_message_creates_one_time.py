...
async def test_process_message_creates_one_time_reminder(monkeypatch):
    ...
    try:
        ...
        # Let the test use the real Ollama call from production code (no monkeypatch)

        # Ensure a daily schedule exists for this user (mirrors real-life state)
        # so the dialogue flow that depends on an existing daily schedule is exercised.
        SchedulerService.create_daily_schedule(user_id, lesson_id=None, time_str="12:00", session=db)

        # Monkeypatch the trigger handler to dispatch a create_schedule with schedule_spec
        ...
        # Call process_message which will invoke the real Ollama client and our fake trigger handler
        resp = await dialogue.process_message(user_id, "Remind me to go out with the garbage at 12:00", db)
        # Print the LLM response so it's visible in test output for inspection
        print("--- LLM response start ---")
        print(resp)
        print("--- LLM response end ---")
        # We don't assert exact LLM wording; ensure we received a non-empty response
        assert isinstance(resp, str) and resp.strip()

        # Verify schedule and stored message
