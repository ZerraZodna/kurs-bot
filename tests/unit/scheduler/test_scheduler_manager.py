...
class TestSchedulerManager:
    ...
    def test_create_get_update_deactivate(self, db_session: Session, test_user, lesson):
        ...
        schedules = manager.get_user_schedules(test_user.user_id, active_only=True, session=db_session)
    ...
    def test_find_active_daily_and_deactivate_user(self, db_session: Session, test_user):
        ...
        found = manager.find_active_daily_schedule(test_user.user_id, session=db_session)

        # Then: Should find an active schedule
        assert found is not None

        # When: Deactivating all user schedules
        count = manager.deactivate_user_schedules(test_user.user_id, session=db_session)
