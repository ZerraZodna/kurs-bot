...
def test_one_time_creation_does_not_modify_daily(db_session):
    ...
    user_id = make_ready_user(db_session, external_id="one_time_test", first_name="One")

    # Given: Create a daily schedule
    daily = SchedulerService.create_daily_schedule(
    ...
    one = SchedulerService.create_one_time_schedule(
        user_id=user_id, run_at=run_at, message="One-time reminder", session=db_session
    )
