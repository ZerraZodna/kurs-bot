    ...
    def test_create_one_time_schedule_converts_to_utc_once(self):
        ...
        with (
            ...
            result = create_one_time_schedule(
                user_id=1, run_at=local_run_at, message="Time to go out with the garbage", session=mock_session
            )
...
class TestDateTriggerBehavior:
    ...
    def test_date_trigger_fires_at_correct_utc_time(self):
        ...
        assert next_fire == future_utc, f"Trigger next fire time mismatch! Expected: {future_utc}, Got: {next_fire}"
...
class TestSchedulerJobTimezoneHandling:
    ...
    def test_to_utc_idempotent(self):
        ...
        assert utc_1 == utc_2, f"to_utc is not idempotent! First call: {utc_1}, Second call: {utc_2}"
