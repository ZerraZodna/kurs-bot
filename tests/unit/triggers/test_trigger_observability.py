"""
Unit tests for Trigger Observability.

Migrated from tests/test_trigger_observability.py to use new test fixtures.
"""
...
def _seed_minimal_triggers(db_session, svc: DummyEmbedSvc):
    """Seed minimal triggers for testing."""
...
class TestTriggerObservability:
    ...
    async def test_paraphrases_map_to_same_schedule_action(self, db_session):
        ...
        Then: All paraphrases should match create_schedule action."""
        svc = DummyEmbedSvc()
        _seed_minimal_triggers(db_session, svc)
    ...
    async def test_explain_match_has_structured_diagnostics(self, db_session):
        ...
        Then: Should return structured diagnostic information."""
        svc = DummyEmbedSvc()
        _seed_minimal_triggers(db_session, svc)
    ...
    async def test_handle_triggers_reports_fallback_path(self, monkeypatch):
        ...
        Then: Should report fallback_path_used in diagnostics."""
    ...
    async def test_debug_trigger_command_returns_explanation(self, monkeypatch):
        ...
        Then: Should return formatted explanation."""
