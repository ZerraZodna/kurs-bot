...
class TestOnboarding:
    ...
    async def test_complete_onboarding_flow(self, db_session):
        ...
        # Given: A new user
        user = User(
            external_id="test_onboarding_complete_flow",
            channel="telegram",
            first_name="Sarah",
            opted_in=True,
        ...
        # Step 5: User provides lesson status (new vs continuing) - says "new"
        response = await dialogue.process_message(user_id, "I'm new to ACIM", db_session)
        assert response is not None
        
        # Step 6: Handle the intro offer (new user gets offer for Lesson 0)
    ...
    async def test_consent_granted_continues_onboarding(self, db_session):
        ...
        # Given: A new user
        user = User(
            external_id="test_onboarding_consent",
            channel="telegram",
            first_name="Alex",
            opted_in=True,
            created_at=datetime.now(timezone.utc),
        ...
        response = await dialogue.process_message(user.user_id, "Hi", db_session)
        response = await dialogue.process_message(user.user_id, "My name is Alex", db_session)
        
        # And: Providing consent
        response = await dialogue.process_message(user.user_id, "Yes", db_session)
        
        # Then: Response should include localized thank-you
        assert "Thank you for consenting" in response
        
        # And: Should continue by asking commitment
        assert (
    ...
    async def test_schedule_request(self, db_session):
        ...
        assert response is not None
        # Name is asked first since user has no name stored
