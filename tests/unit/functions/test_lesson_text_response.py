...
def test_send_todays_lesson_includes_full_content():
    ...
    batch_result = BatchExecutionResult(results=[lesson_result], all_succeeded=True, total_execution_time_ms=50.0)
...
def test_send_lesson_with_content():
    ...
    batch_result = BatchExecutionResult(results=[lesson_result], all_succeeded=True, total_execution_time_ms=45.0)

    built = builder.build(
        user_text="show me lesson 15", ai_response_text="", execution_result=batch_result, include_function_results=True
    )
...
def test_template_formatting():
    ...
    formatted = template.format(lesson_id=20, title="I am determined to see.", content="Full lesson text here...")
