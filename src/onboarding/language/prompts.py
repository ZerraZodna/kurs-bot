...
ONBOARD_MESSAGES["confirm_lesson"] = {
    "en": "The last time we spoke you told me you were on Lesson {lesson_id}. From a gentle, loving place: did you complete that lesson? Reply 'yes' to receive today's lesson, or 'no' if you'd like to continue where you left off.",
...
def get_lesson_confirmation_prompt(language: str, lesson_id: int) -> str:
    """Return a localized confirmation prompt using the onboarding templates.

    Uses `ONBOARD_MESSAGES['confirm_lesson']` and formats the `{lesson_id}`
    placeholder into the localized template. Falls back to English when the
    requested language is not recognized.
    """
