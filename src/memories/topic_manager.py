...
class TopicManager:
    ...
    def get_topic(self, user_id: int, topic: MemoryTopic) -> TopicData:
        ...
        for field_name, memories in field_memories.items():
            ...
            sorted_memories = sorted(
                memories, key=lambda x: x.get("created_at") or datetime.min.replace(tzinfo=timezone.utc), reverse=True
            )
    ...
    def get_name(self, user_id: int) -> str:
        ...
        if all_names:
            ...
            sorted_names = sorted(
                all_names, key=lambda x: x.get("created_at") or datetime.min.replace(tzinfo=timezone.utc), reverse=True
            )
