...
from src.models.database import SessionLocal, User, Schedule, Memory, init_db
...
def dump_user_state(db, user_id):
    ...
    if not user:
        ...
        return
    print('User:', {'user_id': user.user_id, 'external_id': user.external_id, 'timezone': user.timezone, 'first_name': user.first_name, 'lesson': getattr(user, 'lesson', None)})
    ...
    for s in schedules:
        ...
        print('  ' + status + ' [' + s.schedule_type + '] id=' + str(s.schedule_id) + ', cron=' + repr(s.cron_expression) + ', next=' + next_str + ', lesson=' + repr(lesson_title))
    
    print('\\nSchedule memories:')
    relevant_keys = ['schedule_message', 'preferred_daily_time', 'schedule_request_pending']
    sm = db.query(Memory).filter(
...
def direct_create_hourly(db, user_id):
    ...
    print('✓ Created hourly schedule id=' + str(schedule.schedule_id) + ', cron=' + repr(hourly_cron) + ', next=' + schedule.next_send_time.isoformat())
...
def run_hourly_setup(user_id):
    ...
    try:
        ...
        print('python -c \"from src.scheduler.api import execute_scheduled_task; print(execute_scheduled_task(' + str(schedule.schedule_id) + ', simulate=True))\"')
        print('\\nReal send:')
        print('python -c \"from src.scheduler.api import execute_scheduled_task; execute_scheduled_task(' + str(schedule.schedule_id) + ')\"')
